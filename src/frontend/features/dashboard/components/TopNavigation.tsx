...
const TopNavigation: React.FC<TopNavigationProps> = ({ user }) => {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center border-b border-slate-200 bg-white px-4 shadow-sm">
      {/* LEFT: Brand */}
      <Link
        href="/dashboard"
        className="mr-2 flex shrink-0 cursor-pointer items-center gap-2 lg:mr-4 lg:w-48 lg:pl-2"
      >
        <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-600 font-bold text-white shadow-sm text-xs">
          F
        </div>
        <span className="hidden text-lg font-bold tracking-tight text-slate-800 sm:inline">
          Flowva
        </span>
      </Link>

      {/* CENTER-LEFT: Main Navigation */}
      <nav className="mr-auto flex items-center gap-0.5">
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex cursor-pointer items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium whitespace-nowrap transition-all lg:gap-2 lg:px-3 ${
                active
      ...
      <div className="relative mx-2 hidden w-full max-w-sm md:block lg:mx-4">
        <div className="group relative">
          <Search
            className="absolute top-2 left-3 text-slate-400 transition-colors group-focus-within:text-indigo-500"
            size={16}
          />
          <input
            type="text"
            placeholder="Search..."
            className="w-full rounded-full border border-transparent bg-slate-100 py-1.5 pr-4 pl-9 text-sm text-slate-700 shadow-sm transition-all outline-none placeholder:text-slate-400 hover:border-slate-200 focus:border-indigo-300 focus:bg-white focus:ring-4 focus:ring-indigo-100"
          />
          {/* Tooltip hint for "Power User" feature */}
          <div className="pointer-events-none absolute top-2 right-3 hidden group-focus-within:block">
      ...
      <div className="ml-auto flex w-auto shrink-0 items-center justify-end gap-1.5 sm:ml-0 lg:gap-2">
        {/* Project Switcher */}
        <div 
          aria-label="Switch project"
          className="mr-2 hidden cursor-pointer items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 transition-colors hover:border-slate-300 hover:bg-white xl:flex"
        >
          <div className="flex h-5 w-5 items-center justify-center rounded bg-blue-100 text-blue-600">
            <Layers size={12} />
          </div>
          <div className="flex flex-col">
            <span className="text-[10px] leading-none font-bold text-slate-700">Project Alpha</span>
            <span className="mt-0.5 text-[8px] leading-none text-slate-500">Software Dev</span>
          </div>
        ...
        <button 
          ...
          className="relative cursor-pointer rounded-full p-1.5 text-slate-500 transition-colors hover:bg-slate-100"
        >
          <Bell size={18} />
          <span className="absolute top-1.5 right-1.5 h-1.5 w-1.5 rounded-full border border-white bg-red-500"></span>
        </button>
        ...
        <div className="relative">
          <button
            ...
            className={`ml-1 flex cursor-pointer items-center gap-1.5 rounded-full border p-0.5 pl-0.5 transition-colors ${isProfileOpen ? 'border-indigo-100 bg-indigo-50' : 'border-transparent hover:border-slate-200 hover:bg-slate-50'}`}
