...
const ProjectSelector: React.FC<ProjectSelectorProps> = ({
  ...
  // Tạo initials từ tên project (VD: "Project Alpha" → "PA")
      ...
      <div className="animate-in fade-in slide-in-from-bottom-8 w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl delay-100 duration-700">
        <div className="p-8">
          <div className="mb-6 flex items-center justify-between">
            ...
            <button
              ...
              className="flex cursor-pointer items-center gap-1 text-sm font-semibold text-indigo-600 hover:underline"
            >
              ...
              projects.map((project, idx) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`flex h-12 w-12 items-center justify-center rounded-lg ${avatarColors[idx % avatarColors.length]} text-white shadow-sm transition-transform group-hover:scale-110`}
                    >
                      <span className="text-lg font-bold">{getInitials(project.name)}</span>
                    </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-8 py-4">
          <span className="flex items-center gap-1 text-sm text-slate-400">
            <Layers size={14} /> {projects.length} Project{projects.length !== 1 ? 's' : ''}
          </span>
